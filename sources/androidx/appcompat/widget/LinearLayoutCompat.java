...
public class LinearLayoutCompat extends ViewGroup {
    ...
    public void measureHorizontal(int widthMeasureSpec, int heightMeasureSpec) {
                ...
                while (i4 < count) {
                    ...
                    if (child2 == null) {
                        ...
                        if (isExactly) {
                            ...
                            this.mTotalLength = Math.max(maxHeight6, maxHeight6 + largestChildWidth6 + lp3.leftMargin + lp3.rightMargin + getNextLocationOffset(child2));
                        }
                    }
                    i = i2;
                    i4 = i + 1;
        ...
        if (maxAscent[1] == -1 && maxAscent[0] == -1 && maxAscent[2] == -1 && maxAscent[3] == -1) {
            ...
            maxHeight2 = Math.max(maxHeight7, Math.max(maxAscent[3], Math.max(maxAscent[0], Math.max(maxAscent[1], maxAscent[2]))) + Math.max(maxDescent[3], Math.max(maxDescent[0], Math.max(maxDescent[1], maxDescent[2]))));
        }
