...
public class CompoundHash {
    ...
    public static class SimpleSizeSplitStrategy implements SplitStrategy {
        ...
        public SimpleSizeSplitStrategy(Node node) {
            this.splitThreshold = Math.max(512L, (long) Math.sqrt((double) (100 * NodeSizeEstimator.estimateSerializedNodeSize(node))));
        }
    ...
    public static void processNode(Node node, final CompoundHashBuilder state) {
        if (node.isLeafNode()) {
            ...
            ((ChildrenNode) node).forEachChild(new ChildrenNode.ChildVisitor() {
                /* class com.google.firebase.database.snapshot.CompoundHash.AnonymousClass1 */
